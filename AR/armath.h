// AR math routines.
// (C) 2012 Julien Cayzac <julien@cayzac.name>
...
static inline Matrix4 GetPerspectiveProjectionMatrix(
	const double& noalias(near),
	const double& noalias(far),
	...
	return Matrix4(
		Vector4(focal,              0,                      0,  0),
		Vector4(    0, aspect * focal,                      0,  0),
		Vector4(    0,              0,   (far + near) * invNF, -1),
		Vector4(    0,              0, 2 * far * near * invNF,  0)
	);
}

static inline Matrix4 GetInfiniteProjectionMatrix(
	const double& noalias(near),
	const double& noalias(fov),
	const double& noalias(aspect)
) {
...
static inline void GetSinesFromAngles(
	const double& noalias(phi),
...
static inline void GetSinesFromAnglesInDegrees(
	const double& noalias(phi),
	const double& noalias(lambda),
	double& noalias(cosineOfPhi),
	double& noalias(sineOfPhi),
...
static inline void GetECEFFromSinesAndAltitude(
	const double& noalias(cosineOfPhi),
	const double& noalias(sineOfPhi),
	const double& noalias(cosineOfLambda),
	const double& noalias(sineOfLambda),
	const double& noalias(altitude),
	double& noalias(x),
	double& noalias(y),
	double& noalias(z)
) {
...
static inline void GetECEFFromGeodeticCoordinates(
	const double& noalias(latitude),
	const double& noalias(longitude),
	const double& noalias(altitude),
	double& noalias(x),
	double& noalias(y),
	double& noalias(z)
	...
	GetSinesFromAnglesInDegrees(latitude, longitude, cosineOfPhi, sineOfPhi, cosineOfLambda, sineOfLambda);
	GetECEFFromSinesAndAltitude(cosineOfPhi, sineOfPhi, cosineOfLambda, sineOfLambda, altitude, x, y, z);
}

// e.g. On iOS,
// Quat cameraRotation(Quat::identity());
//
// // Use CMAttitudeReferenceFrameXTrueNorthZVertical when starting the motion manager
...
// shader.
static inline Matrix4 GetECEFToViewMatrix(
	const Matrix4& noalias(cameraRotation),
	...
	GetSinesFromAnglesInDegrees(cameraLatitude, cameraLongitude, cPhi, sPhi, cLambda, sLambda);

	double rX, rY, rZ;
	GetECEFFromSinesAndAltitude(cPhi, sPhi, cLambda, sLambda, cameraAltitude, rX, rY, rZ);
