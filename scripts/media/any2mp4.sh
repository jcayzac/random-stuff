...
TIMESTAMP=$( exiftool -s -d '%s' "$1"|grep -E '(DateTime|CreateDate)' |sed 's/.*: //g'|sort|head -n 1 )
if [ -z "$TIMESTAMP" ]
then
...
ffmpeg -i "$1" -y -f mp4 -vcodec libx264 -crf 20 -threads 2 -flags +loop -cmp +chroma -deblockalpha -1 -deblockbeta -1 -refs 3 -bf 3 -coder 1 -me_method hex -me_range 18 -subq 7 -partitions +parti4x4+parti8x8+partp8x8+partb8x8 -g 320 -keyint_min 25 -level 41 -qmin 10 -qmax 51 -qcomp 0.7 -trellis 1 -sc_threshold 40 -i_qfactor 0.71 -flags2 +mixed_refs+dct8x8+wpred+bpyramid -padcolor 000000 -padtop 0 -padbottom 0 -padleft 0 -padright 0 -acodec libfaac -ab 192kb -ar 48000 -ac 2 $YEAR -metadata comment="$BASE" -timestamp now "$TEMPORARY"
STATUS=$?
if [ $STATUS != 0 ]
then
	echo FAILED
